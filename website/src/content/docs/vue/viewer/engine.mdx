...
Use the `@ready` event to access the plugin registry, then get the engine:
...
<template>
  <button @click="getDocumentMetadata">Get Metadata</button>
  <PDFViewer 
    @init="handleInit"
    @ready="handleReady"
    :config="{ src: 'https://raw.githubusercontent.com/arnon3339/pubme/main/pdf/Digital_Inno_Cover.pdf' }" 
  />
</template>
```
...
<CodeExample
  codePaths={[
    "../../examples/vue-tailwind/src/examples/viewer/engine-example.vue"
  ]}
>
  <EngineExample />
</CodeExample>
...
- **Add annotations** → Use the [Annotation Plugin](/docs/vue/headless/plugins/plugin-annotation)
- **Change zoom** → Use the [Zoom Plugin](/docs/vue/headless/plugins/plugin-zoom)
- **Navigate pages** → Use the [Scroll Plugin](/docs/vue/headless/plugins/plugin-scroll)
